...
const SearchByNameId: React.FC = () => {
    ...
    useEffect(() => {
        const savedNeuron = localStorage.getItem('neuromorph_search_neuron');
        if (savedNeuron) {
            try {
                setFoundNeuron(JSON.parse(savedNeuron));
            } catch (err) {
                console.error('Failed to parse saved neuron data:', err);
    ...
    useEffect(() => {
        if (foundNeuron) {
            localStorage.setItem('neuromorph_search_neuron', JSON.stringify(foundNeuron));
        }
    }, [foundNeuron]);
